...
目前，现有的空间表本身就是对空间的抽象，私有空间和团队空间，属于不同的空间类别。要想区别这两者，在原始的空间表中添加spaceType字段即可。

私有空间和团队空间的另一大区别是：**团队空间具有成员的概念**。

成员实际上是用户与团队空间之间的关系。新建表即可。

* 权限校验
  1. 现有的权限校验是使用**注解+AOP**的方式，对于团队权限校验，也完全可以采用这种方式（因为团队空间的权限和角色较少）。
  2. 使用权限校验框架来实现。这里处于学习的目的，采用本方法。

### RBAC权限校验模型(Role-Based access controll)

RBAC的核心概念有：**用户，角色，权限**。

一个用户可以有多个角色，一个角色可以被多个用户担任。

一个角色可以有多个权限，一个权限可以被多个角色所拥有。
...
digraph RBAC_Model {
    ...
    // 定义节点
    用户 [label="{<f0> 用户 | <f1> • 用户ID: BIGINT | <f2> • 用户名: VARCHAR}"];
    角色 [label="{<f0> 角色 | <f1> • 角色ID: BIGINT | <f2> • 角色名称: VARCHAR | <f3> • 描述: VARCHAR}"];
    权限 [label="{<f0> 权限 | <f1> • 权限ID: BIGINT | <f2> • 权限名称: VARCHAR | <f3> • 描述: VARCHAR}"];
